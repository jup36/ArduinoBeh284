...
void loop() {
  if (Serial.available() > 0){
    //grace_period = Serial.parseInt(); 
    //waterDur = Serial.parseInt(); 
    //airDur = Serial.parseInt(); 
    ...
    if (serialData >= '0' && serialData <= '9') {
      // If the character is numeric, parse the integer
      Serial.unread(serialData); // Unread the character to allow parseInt() to read it
      grace_period = Serial.parseInt(); 
      waterDur = Serial.parseInt(); 
      airDur = Serial.parseInt(); 
    }
  ...
  int switchState = digitalRead(switchPin);   
    ...
    if ((millis() - lastWaterDelivered > 500) && (millis() - lastAirPuffDelivered > 500)  && (millis() - stimStart > grace_period) && (serialData == 'R' || serialData == 'P')) {
