...
// Pulse parameters
const unsigned long waterDur = 50;   // Duration of the water pulse (ms)
...
void loop() {
  ...
  int switchState = digitalRead(switchPin);   
    ...
    if ((millis() - stimStart > 500) && (serialData == 'R' || serialData == 'P')) {
      // Check if the lickometer state has changed
      if (lickState == HIGH) {
        Serial.println(7);
        if (lastLickStateChange == 0) {
        ...
        if (lickPulseWidth > 5 ) { //&& lickPulseWidth < 500
          // Pulse detected, trigger the output pulse
          Serial.println(9);
          if ((serialData == 'R') && waterDelivered == false) {
            digitalWrite(waterPin, HIGH);
            delay(waterDur);
            digitalWrite(waterPin, LOW);
            waterDelivered = true;
