...
const unsigned long airDur = 30;   // Duration of the air pulse (ms)

// Temporal parameters
unsigned long waterGrace = 50;
unsigned long airGrace = 500;
unsigned long waterInterval = 1000;
unsigned long airInterval = 3000; 
...
void loop() {
  ...
  int switchState = digitalRead(switchPin);   
    ...
    if (serialData == 'R'){ // water trial
      if ((millis() - stimStart > waterGrace) && (millis() - lastWaterDelivered > waterInterval)) {
        ...
        if (lickState == HIGH && lastLickStateChange == 0) {
          ...
          if (lickPulseWidth > 5 ) { //&& lickPulseWidth < 500
            ...
            Serial.println(1);  
            lastLickStateChange = 0; // Reset the variables for the next pulse detection 
          }
        }
      }
    } else if (serialData == 'P'){ // air trial
      if ((millis() - stimStart > airGrace) && (millis() - lastAirPuffDelivered > airInterval)) {
        // Check if the lickometer state has changed
        if (lickState == HIGH && lastLickStateChange == 0) {
          lastLickStateChange = millis(); // Record the time of the lickometer rising edge
        } else if (lickState == LOW && lastLickStateChange > 0) {
