...
void loop() {
  ...
  int switchState = digitalRead(switchPin);   
    ...
    if ((millis() - lastWaterDelivered > 1000) && (millis() - lastAirPuffDelivered > 1000)  && (millis() - stimStart > 200) && (serialData == 'R' || serialData == 'P')) {
      ...
      if (lickPulseWidth > 5 ) { //&& lickPulseWidth < 500
        ...
        if (serialData == 'R' && dropDelivered == false) {
          ...
          Serial.println(1);
          dropDelivered = true;
        } else if ((serialData == 'P' && airpuffDelivered == false) ) { //&& airpuffDelivered == false
